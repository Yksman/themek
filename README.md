...
**수집 파이프라인(#1·#3·#4·#5·#6) + KRX sync + Graph Core 온톨로지 + Vault 투영 + 파이프라인 통합 + 재무 정합성(Track A) + 그래프 연결·해소·확장(Track B) + 탐색 가시성(Track C) + 지분구조(Equity Ownership) 구현 완료** — 2026-06-01. (테스트 351개 전부 통과)
...
- Equity Ownership Ontology ✅ 2026-06-01 — `OWNS_STAKE_IN`(person|company→company) + DART 최대주주/타법인출자 정형 적재(`themek equity ingest`) + 외부법인/오너 해소 + 지분구조·people vault 투영 + 39개사 프로덕션 검증(`themek equity verify` → coverage 1.0 / 4424 edges / 399 person, 삼성생명 8.51%·이재용 1.63% 공시값 일치)

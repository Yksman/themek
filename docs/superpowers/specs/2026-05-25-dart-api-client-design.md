...
date: 2026-05-25
status: Draft v1.1 (실 API 직진 방침 확정 — plan 작성 진입)
scope: DART OpenAPI 기반 사업보고서 자동 fetch + 캐시 + ingest 통합. 다종목·다기간 backbone (전 종목 backfill은 Plan #5 위임).
revision_notes:
  - v1.1 (2026-05-25): DS003 검증 결과 반영 — 매출 부문/고객/지역 structured endpoint 부재 확정 → zip + HTML 휴리스틱 + LLM 추출 경로 유지. HTTP 테스트는 stub이 아닌 cassette(vcrpy) 채택. T0를 "실 API 정찰" task로 재정의.
---
...
- typer (CLI)
- **테스트 HTTP**: `vcrpy` cassette (실 응답 1회 저장 후 playback) — dev 의존성

**D2 (확정):** `vcrpy` 채택. 이유: (1) 사용자 요구 "실 API로 개발·검증" 부합, (2) cassette가 git tracked면 CI/타인 머신에서 `DART_API_KEY` 없이도 테스트 통과, (3) 응답 구조가 손-작성 fixture와 괴리될 위험 제거.
...
### 11.1 개발 워크플로 (실 API 직진)

기존 spec의 "stub-first"를 폐기하고 **cassette playback**으로 전환:

```
[T0 정찰 단계 — 실 API 1회]
   ↓
1. DART_API_KEY로 corpCode.xml zip 1회 fetch → 디스크 저장 + 응답 구조 분석
2. list.json (corp_code=00126380, year=2023) 1회 fetch → rcept_no 확인
3. document.xml (rcept_no=20240314000123) 1회 fetch → zip 받아 내부 HTML 구조 분석
4. 위 3개 응답을 cassette로 저장 (tests/fixtures/dart_cassettes/)

[T1+ 본 개발]
   ↓
- 모든 테스트는 cassette playback (실 API 0회 hit)
- 코드 변경 후 응답 가정 깨지면 cassette refresh (`pytest --vcr-record=new_episodes`)
```
...
| 레이어 | 케이스 수 | HTTP 처리 |
|--------|----------|----------|
| `DartClient` HTTP 경로 | 5 (success·rate·auth·5xx·timeout) | vcr cassette 1건 success + monkeypatch로 에러 분기 |
| `DartCache` 파일 IO | 4 (save·load·hit·miss) | N/A |
| `corp_lookup` zip parse + lookup | 3 (sync·hit·miss) | cassette (corpCode.xml zip) |
| `fetch.extract_business_html_from_zip` | 3 (heuristic match·fallback·fail) | 실 document.xml zip cassette |
| `fetch.find_business_report_rcept_no` | 3 (match·multi-match→latest·no-match) | 실 list.json cassette |
| `fetch.fetch_business_report_html` 오케스트레이션 | 2 (cache miss → fetch 1회, cache hit → 0 호출) | cassette + tmp_path |
| CLI `dart ingest` 통합 | 2 (perfect path, missing ticker) | cassette + tmp DB |

총 ~22개, **모두 실 hit 0회** (T0에서 만든 cassette만 재생).

### 11.3 Cassette 저장 정책

- 위치: `tests/fixtures/dart_cassettes/<test_name>.yaml` (vcr 기본 포맷)
- git tracked (공개 정보, 단일 보고서 zip 압축 후 cassette 파일 ~수백KB 예상)
- API key는 cassette 저장 시 자동 redact (`filter_query_parameters=['crtfc_key']`)
- refresh 정책: spec 변경 또는 DART 응답 schema 변동 시 명시적 `--vcr-record=new_episodes` 1회 재실행
- CI/타인 머신: cassette만으로 테스트 통과 (DART_API_KEY 불필요)
...
v1.1 시점: D2 / D6 확정. 나머지는 권장 default 채택 — plan 작성 중 변경 시 별도 commit.

| ID | 결정 사항 | 결정 | 비고 |
|----|----------|------|------|
| D1 | 사업보고서 본문 fetch 방식 | ✅ `document.xml` zip + 휴리스틱 HTML 선택 | DS003 검증으로 structured endpoint 부재 확인 |
| D2 | 테스트 HTTP 방식 | ✅ **vcrpy cassette** | 실 API 직진 + cassette playback |
| D3 | `data/dart/corp_master.json` git tracked? | ✅ NO (gitignore) | sync-corp 1회 실행 전제 |
| D4 | `report-type` MVP 범위 | ✅ 사업보고서만 | 반기/분기는 Plan #5 |
| D5 | `Corporation.dart_corp_code` migration | ⏳ T0에서 model 확인 후 결정 | 있으면 noop, 없으면 alembic 1건 |
| D6 | corp_lookup 데이터 출처 | ✅ `corp_master.json` (sync-corp 선행) | seed 직접 사용 시 다종목 확장 막힘 |
| D7 | retry 횟수·백오프 | ✅ 3회 / 1s·2s·4s | jitter 미사용 |
| D8 | DART_API_KEY env 키 이름 | ✅ `DART_API_KEY` | prefix 없음 (외부 표준 키) |
...
`docs/superpowers/plans/2026-05-25-dart-api-client.md` 작성 — 12-15 task TDD 구조:

- **T0: 실 API 정찰** — `DART_API_KEY` 등록 + `corpCode.xml` zip 1회 fetch + 삼성전자 사업보고서 zip 1회 fetch + 응답 3건을 cassette로 저장 + zip 내부 구조 보고서 작성 (`docs/dart-api-recon-notes.md`). `Corporation.dart_corp_code` 컬럼 존재 확인 → D5 확정.
- T1-T3: `client.py` (corpCode·list·document 3 메서드) + cassette playback 테스트 + 에러 분기
- T4: `cache.py` + 테스트
- T5-T6: `corp_lookup.py` (zip 파싱 + ticker → corp_code 조회) + 테스트
- T7-T8: `fetch.py` — `extract_business_html_from_zip` (휴리스틱) + `find_business_report_rcept_no` (필터)
- T9: `fetch.fetch_business_report_html` 오케스트레이션 (cache hit/miss + 통합)
- T10-T11: CLI 3개 명령 (`sync-corp` / `fetch` / `ingest`) + 통합 테스트
- T12: 실 LLM smoke run — 005930·005380·277810 3종목에 대해 `dart ingest` → `query e5` → `eval e5` 일관 동작 검증
- T13: `.gitignore` 갱신 + `docs/dart-fetch-smoke-run-notes.md` baseline
- T14: README "후속 Plan들" Plan #3 ✅ 갱신

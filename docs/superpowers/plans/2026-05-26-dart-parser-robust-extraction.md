...
- [x] **Step 1: 실패 테스트 작성** (`tests/test_parser_escalation.py` 신규)
...
- [x] **Step 3: `SectionResolution` 확장** (`src/themek/dart/parser.py`)
...
- [x] **Step 1: 실패 테스트 추가** (`tests/test_parser_escalation.py` 끝)
...
- [x] **Step 3: `parser.py`에 `MIN_SECTION_CHARS` 상수 + 체크 추가**
...
- [x] **Step 3: `extract_business_sections` 분기 갱신**
...
- [x] **Step 3: `extract_business_sections` 끝부분에 fallback 추가**
...
- [x] **Step 5: Rainbow 실 HTML로 end-to-end 회귀 확인**
...
- [x] **Step 1: 실패 테스트 추가** (`tests/test_cli_eval.py` 끝)
...
- [x] **Step 3: `_format_section_log` 갱신** (`src/themek/cli.py`)
...
- [x] **Step 4: `dart_ingest_cmd`에 stdout 한 줄 추가**
...
- [x] **Step 1: 실패 테스트 작성** (`tests/test_learned_patterns.py`)
...
- [x] **Step 3: `src/themek/dart/learned_patterns.py` 구현**
...
- [x] **Step 5: 초기 `data/dart/learned_header_patterns.json` 작성**
...
- [x] **Step 1: 실패 테스트 추가** (`tests/test_parser_sections.py` 끝)
...
- [x] **Step 3: parser.py 보강 — `LearnedPatterns`를 매 호출마다 로드**
...
- [x] **Step 3: `extract_business_sections`에서 LLM 결과 처리 시 sample 기록**
...
- [x] **Step 3: `src/themek/dart/pattern_learning.py` 구현**
...
- [x] **Step 3: `validate_pattern_against_fixtures` 구현**
...
- [x] **Step 5: `tests/fixtures/dart_variants/.gitkeep` 추가**
...
- [x] **Step 3: `Proposal` + record/load/save 구현**
...
- [x] **Step 5: 초기 `data/dart/pattern_proposals.json` 생성**
...
- [x] **Step 3: `apply_ready_proposals` 구현**
...
- [x] **Step 3: `cli.py:dart_ingest_cmd` 끝에 학습 hook 추가**
...
- [x] **Step 1: 실패 테스트 추가** (`tests/test_cli_dart.py` 끝)
...
- [x] **Step 3: `dart_ingest_cmd`에 mirror 호출 추가**
...
- [x] **Step 3: fixture를 사용한 회귀 검증 테스트 추가** (`tests/test_parser_sections.py`)
...
- [x] **Step 1: 실패 테스트 작성** (`tests/test_cli_parser_commands.py`)
...
- [x] **Step 3: `cli.py`에 parser-stats 추가**
...
## Plan #4 Completion Notes (2026-05-27)

- Tasks 1-19: 코드 + 테스트 완료, 모두 commit 됨 (b0491e8 ~ 2b2276e)
- Task 20 (신규 5종목 ingest로 fixture coverage 확장): **deferred** — 별도 작업으로
  분리. 현재 fixtures/dart_variants에 005380만 실 cache mirror, 005930·277810은
  stub 상태. coverage 확장은 실 `claude` CLI baseline run 시점에 동시 진행 권장.
- Task 21: notes 대신 status 문서로 대체 — `docs/dart-parser-robust-extraction-status-2026-05-27.md`
